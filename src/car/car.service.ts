...
export class CarService {
  ...
  async sortByPrice({ order = 1, limit = 0, ...query } = {}): Promise<Car[]> {
    ...
    return await this.carRepository.find({ where: query, order: { price_day: sort }, take: limit })
  }
  ...
  async sortByYear({ order = 1, limit = 0, ...query } = {}): Promise<Car[]> {
    ...
    return await this.carRepository.find({ where: query, order: { year: sort }, take: limit })
  }
