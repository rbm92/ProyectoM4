...
let RentalService = class RentalService {
    ...
    async create(rentalDto) {
        ...
        try {
            if (!car.available)
                throw new common_1.HttpException('car not available for rental', common_1.HttpStatus.BAD_REQUEST);
            if (user.active_rental)
                throw new common_1.HttpException('this user already has an active rental', common_1.HttpStatus.BAD_REQUEST);
            car.available = false;
    ...
    async findRentals(query) {
        try {
            ...
            throw new common_1.HttpException('rental not found', common_1.HttpStatus.BAD_REQUEST);
        }
    }
    ...
    async sortByReturnDate(_a = {}) {
        var { order = 1, limit = 0 } = _a, query = __rest(_a, ["order", "limit"]);
        try {
    ...
    async findRentalsOneUser(id) {
        try {
            ...
            throw new common_1.HttpException('user has no rental history', common_1.HttpStatus.BAD_REQUEST);
        }
    }
    async findRentalsOneCar(id) {
        try {
            ...
            throw new common_1.HttpException('car has no rental history', common_1.HttpStatus.BAD_REQUEST);
        }
    }
    async update(id, rentalDto) {
        try {
            ...
            let newRental;
            if (!user.active_rental)
                throw new common_1.HttpException('this user has no active rentals', common_1.HttpStatus.BAD_REQUEST);
            if (car.available)
                throw new common_1.HttpException('this car is already available', common_1.HttpStatus.BAD_REQUEST);
            car.available = true;
